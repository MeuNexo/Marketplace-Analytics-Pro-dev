-- ============================================================
-- Phase 68 alvo order-up-to — corrige alvo < ponto quando lead > meta_cobertura
--
-- Herdado da 20260668000200 (motor simples=espinha, inteligente só em ALTA CONFIANÇA).
-- Mudança ÚNICA: o termo de dias do alvo (order-up-to) usa
--   GREATEST(meta_cobertura_dias, lead_time_dias + 7)
-- onde 7 = ciclo de revisão (reposição a cada 7 dias).
--
-- Problema corrigido: quando meta_cobertura_dias (ex.: 45) é menor que lead_time_dias
-- (ex.: 70/72 Pralana, até 85 em outras marcas), o alvo caía abaixo do ponto de reposição.
-- Nesses casos: estoque+a_caminho entre alvo e ponto → gatilho dispara mas a compra
-- colapsa no MOQ. Com GREATEST, alvo >= ponto é garantido estruturalmente.
--
-- Impacto: apenas SKUs cujo lead_time_dias > meta_cobertura_dias recebem alvo maior;
-- para os demais, GREATEST mantém o comportamento anterior (meta_cobertura_dias vence).
--
-- SECURITY INVOKER, assinatura 4-arg. Mesmo RETURNS TABLE (35 colunas) da 200.
-- ============================================================

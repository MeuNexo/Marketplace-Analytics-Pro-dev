-- 243-01 — frete gratis PROVADO deixa de ser duvida e vira conta fechada
--
-- 🔴 ACHADO ao analisar a aba "Nosso erro" a pedido do Wesley. Das 14 linhas da
-- fila, 11 nao tinham nada a corrigir — e 2 delas eram frete gratis com as tres
-- fontes concordando:
--
--   nossa base, custo publicado do envio  ->  R$ 0,00
--   nossa base, captura de cobranca       ->  `ok`, tentada HOJE
--   o Mercado Livre, consultado ao vivo   ->  so CVVML e CVVPRC; ZERO frete
--
-- A conta fecha em 0,00 / 0,00 / 0,00 — e o card dizia "pode ser frete gratis
-- OU lacuna da nossa captura, nao presumimos zero", mandando corrigir quem nao
-- tem o que corrigir.
--
-- "Nao presumimos zero" era a regua CERTA enquanto nao tinhamos perguntado.
-- Agora perguntamos. E o mesmo defeito das fases 239-242 na direcao contraria:
-- ter a prova e nao usar.
--
-- ⚠️ AS TRES PROVAS, NUNCA DUAS. Sem a captura recente, "esperado zero" sozinho
-- e presuncao — foi assim que a 242 errou 39 de 40.
--
...
mot as (
  select c.*,
         ...
         case
           ...
           -- 🔴 243-01: FRETE GRATIS PROVADO, com as TRES provas.
           --    (1) o custo publicado do envio e zero;
           --    (2) existe captura de cobranca tentada para o pedido, e RECENTE;
           --    (3) ela nao trouxe nenhuma linha de frete.
           --    Conferido ao vivo em 05/09/2026 nos pedidos 2000017941686826 e
           --    2000017945693374: o ML devolve so `CVVML` e `CVVPRC`, nenhuma
           --    linha de frete. A conta FECHA em 0,00 / 0,00 / 0,00.
           --
           -- ⚠️ AS TRES, NUNCA DUAS. Sem a captura recente, "esperado zero"
           --    sozinho e presuncao — foi exatamente assim que a 242 errou 39
           --    de 40 ao afirmar que o ML nao tinha emitido.
           when c.n_frete = 0
            and c.list_cost = 0
            and cap.ml_order_id is not null
            and cap.ultima_tentativa::date >= ((select hoje from cfg) - 1)
                                                      then 'frete_gratis_confirmado'
...
select k.id                                                as caso_id,
       ...
       case
         ...
         -- 🔴 243-01: `frete_gratis_confirmado` NAO entra em "nosso". A aba
         -- promete "cadastro ou ingestao nossa para arrumar", e aqui a conta
         -- fecha: nao ha nada a corrigir.
       ...
       case when m.motivo = 'frete_gratis_confirmado' then 0::numeric
            else m.dif
       end                                                 as residuo_ml,
       ...
       -- 🔴 243-01: no frete gratis confirmado o cobrado e ZERO MEDIDO, nao
       -- ausencia: perguntamos ao ML e ele nao cobrou. Emitir nulo aqui jogaria
       -- a linha no balde "nao apurado" da tela, que e para duvida.
       case when m.motivo = 'frete_gratis_confirmado' then 0::numeric
            when m.e_lider then round(m.cobrado, 2)
       end                                                 as recebido,

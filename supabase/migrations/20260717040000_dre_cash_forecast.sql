-- Phase 100 (Break-even de caixa do mês) — Plan 01, 2026-07-17.
--
-- RPC única `get_dre_cash_forecast(p_org_id uuid, p_month date)` — backend
-- inteiro da Phase 100 (BEC-01 + BEC-04 lado servidor). Compõe com as réguas
-- de caixa VIVAS da Phase 99 (20260717030000_cash_inflows_refund_date.sql):
-- entrada base-cheia por release_date, estorno por COALESCE(refund_date,
-- release_date). Devolve num único round-trip:
--   - saída prevista do mês (pagas + estornos ocorridos + pendentes do mês
--     corrente + estornos previstos + imposto previsto restante)
--   - entrada garantida do mês (liberadas + agendadas, dado real MP)
--   - taxas MEDIDAS dos dados (nunca hardcoded — D-03 do 100-CONTEXT)
--   - lag de liberação estimado (método documentado abaixo, clamp [7,30])
--   - ritmo real (vendas brutas 7d)
--   - guarda anti-fantasma: pendentes limitados ao mês corrente (D-06a) +
--     alerta de recorrência suspeita entre pendentes (D-06b)
--
-- Decisão travada 100-CONTEXT/Backend (D-07): a META NÃO entra na RPC — é
-- aplicada client-side sobre o gap (D-04). O ROADMAP original citava um 3º
-- parâmetro de meta na assinatura; essa decisão do dono, capturada depois,
-- supera o ROADMAP — a assinatura fica com 2 argumentos, igual get_dre_cash.
--
-- Padrão canônico idêntico à Phase 99 (nunca "SECURITY DEFINER" — o padrão
-- do projeto com DEFINER + org param como argumento é IDOR; aqui é sempre
-- SECURITY INVOKER, RLS org-first decide o acesso real): LANGUAGE sql,
-- STABLE, SECURITY INVOKER, SET search_path TO 'public'; REVOKE PUBLIC/anon
-- + GRANT authenticated ao final. Todas as CTEs AS MATERIALIZED. ZERO
-- subquery correlacionada (timeout de 8s do role authenticated) — todo
-- lookup pré-carregado em CTE + JOIN.
--
-- CREATE FUNCTION direto (função nova, nunca existiu — se precisar re-rodar,
-- trocar para CREATE OR REPLACE; NUNCA DROP FUNCTION, que apaga a ACL —
-- lição crítica do projeto: feedback_drop_function_apaga_acl.md).
--
-- Aplicar via Supabase MCP apply_migration no projeto ckcdevcxgvueywivefgx
-- (o real desta plataforma; ignorar o project-id desatualizado do
-- CLAUDE.md) — NUNCA supabase db push, NUNCA SQL Editor. Conferir
-- max(version) vivo antes de aplicar (lição 2026-07-13: MCP live usa
-- timestamp real). NÃO aplicar em prod neste commit — orquestrador aplica
-- via MCP (Task 2 do plano), conferindo max(version) vivo antes e
-- produzindo as 8 provas do checkpoint (performance <8s, anti-IDOR com org
-- Thales real, reconciliação ao centavo com get_dre_cash, guarda
-- anti-fantasma ao vivo, taxas em faixa sã, advisors limpos).

CREATE FUNCTION public.get_dre_cash_forecast(
  p_org_id uuid,
  p_month  date
)
RETURNS TABLE (
...
AS $function$
  ...
  taxas_estornos_calc AS MATERIALIZED (
    ...
    WHERE ci.organization_id = p_org_id
      AND ci.net_amount < 0
      AND COALESCE(ci.refund_date, ci.release_date) >= p.janela_ini
      AND COALESCE(ci.refund_date, ci.release_date) <= p.hoje
  ),

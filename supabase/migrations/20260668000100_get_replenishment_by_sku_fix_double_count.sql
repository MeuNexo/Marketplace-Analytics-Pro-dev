-- ============================================================
-- Phase 68 — Fix dupla-contagem qtd_a_caminho: get_replenishment_by_sku v8
--
-- ROOT CAUSE (identificado em debug incoming-by-sku-double-count):
--   incoming_by_sku CTE agrega por po.sku → 1 linha por sku_code.
--   inventory_by_sku CTE tem 1 linha por (item_id, variation_id).
--   base CTE fazia LEFT JOIN incoming_by_sku ON inc.sku_code = inv.sku_code
--   → join N:1: quando N anúncios/variações compartilham o mesmo SKU, cada
--   linha recebia qtd_a_caminho INTEIRA → display duplicado + compra_sugerida
--   e gatilho_ativo subtraindo N vezes o valor real (subcompra).
--
-- FIX:
--   Inserir dois CTEs após incoming_by_sku:
--     sku_share           — conta n_var e soma total_stock por sku_code
--     incoming_per_variation — distribui qtd_a_caminho proporcional ao estoque
--                           de cada variação; fallback igual se total_stock=0;
--                           intacto (=total) quando n_var=1 (sem regressão).
--   base CTE passa a JOIN com incoming_per_variation em (item_id, variation_id)
--   → join 1:1, sem duplicação.
--
...
--   - p_smart=FALSE reproduz Phase 66 exceto pelo fix de dupla-contagem
--   - Colunas de saída idênticas (nenhuma adição/remoção)
--   - Ordem ORDER BY idêntica
--   - Regressão SKU em 1 anúncio: n_var=1 → qtd_a_caminho inteira (= antes)
-- ============================================================
...
BEGIN
  ...
  -- Total de OCs em trânsito por sku_code (mantido como base para o rateio)
  incoming_by_sku AS (
    SELECT po.sku AS sku_code, SUM(po.quantidade)::INTEGER AS qtd_a_caminho_total,
  ...
  -- FIX dupla-contagem: conta variações e soma estoque por sku_code
  -- Usado para distribuir qtd_a_caminho proporcionalmente
  sku_share AS (
    SELECT
      inv.sku_code,
      COUNT(*)::INTEGER                   AS n_var,
      SUM(GREATEST(inv.sku_stock, 0))::NUMERIC AS total_stock
    FROM inventory_by_sku inv
    WHERE inv.sku_code IS NOT NULL AND inv.sku_code <> ''
    GROUP BY inv.sku_code
  ),
  -- FIX dupla-contagem: distribui qtd_a_caminho por variação proporcional ao estoque.
  -- Garante que SUM(qtd_a_caminho) por SKU ≈ qtd_a_caminho_total do SKU (±1 por arredondamento).
  -- Regras:
  --   n_var = 1           → inteiro (sem rateio, sem regressão)
  --   total_stock = 0     → divisão igual entre variações (FLOOR para não exceder total)
  --   total_stock > 0     → proporcional ao estoque da variação (ROUND)
  --   sku_code NULL/empty → 0 (sem OC atribuível)
  incoming_per_variation AS (
    SELECT
      inv.item_id,
      inv.variation_id,
      COALESCE(
        CASE
          WHEN inc.qtd_a_caminho_total IS NULL  THEN 0
          WHEN ss.n_var <= 1                     THEN inc.qtd_a_caminho_total
          WHEN ss.total_stock = 0                THEN FLOOR(inc.qtd_a_caminho_total::NUMERIC / ss.n_var)::INTEGER
          ELSE ROUND(inc.qtd_a_caminho_total::NUMERIC * GREATEST(inv.sku_stock, 0)::NUMERIC / ss.total_stock)::INTEGER
        END,
        0
      )::INTEGER AS qtd_a_caminho,
      inc.data_proxima_chegada
    FROM inventory_by_sku inv
    LEFT JOIN incoming_by_sku inc
      ON inc.sku_code = inv.sku_code
      AND inv.sku_code IS NOT NULL AND inv.sku_code <> ''
    LEFT JOIN sku_share ss ON ss.sku_code = inv.sku_code
  ),
  ...
  base AS (
    SELECT
      ...
      -- FIX: join em (item_id, variation_id) via incoming_per_variation (1:1)
      -- em vez de join em sku_code via incoming_by_sku (N:1 → dupla-contagem)
      ipv.qtd_a_caminho,
      ipv.data_proxima_chegada,
      ...
      CASE
        ...
        WHEN (inv.sku_stock + ipv.qtd_a_caminho)::NUMERIC > ss.avg_daily * p_demand_multiplier * (pr.lead_time_dias + pr.safety_days)::NUMERIC THEN 0
        ELSE GREATEST(CEIL(GREATEST(0, ss.avg_daily * p_demand_multiplier * (pr.meta_cobertura_dias + pr.safety_days)::NUMERIC - (inv.sku_stock + ipv.qtd_a_caminho)::NUMERIC) / NULLIF(pr.pack_multiple, 0)) * pr.pack_multiple, pr.moq)::INTEGER
    ...
    -- FIX: incoming_per_variation em vez de incoming_by_sku
    LEFT JOIN incoming_per_variation ipv ON ipv.item_id = inv.item_id AND ipv.variation_id IS NOT DISTINCT FROM inv.variation_id
    LEFT JOIN LATERAL (

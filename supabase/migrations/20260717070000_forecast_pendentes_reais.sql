-- Phase 100 FIX (confronto do dono, 2026-07-17): (1) 'Previsões de compra' fora dos
-- pendentes do forecast e da guarda anti-fantasma (não são contas comprometidas);
-- (2) imposto previsto zera quando há guia paga OU pendente no mês (dupla contagem).
-- CREATE OR REPLACE, corpo = 20260717060000 + estes pontos.
...
AS $function$
  ...
  saidas_pendentes_agg AS MATERIALIZED (
    ...
    WHERE co.organization_id = p_org_id
      ...
      -- [FIX 2026-07-17] 'Previsões de compra' = previsão sintética de reposição, não conta comprometida
  ...
  imposto_guia_mes AS MATERIALIZED (
    SELECT
      ...
      -- [FIX 2026-07-17] guia PENDENTE já está em saidas_pendentes — previsão zera também
  ...
  SELECT 'saida_prevista'::text, 'imposto_previsto_restante'::text,
         CASE WHEN igm.total_qualquer > 0 THEN 0 ELSE COALESCE(pc.taxa_media * fm.total, 0) END,
         pc.n_validas

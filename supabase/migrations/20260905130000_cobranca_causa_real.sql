-- ════════════════════════════════════════════════════════════════════════════
-- 239-04 · conciliacao_base_linhas — as quatro causas e o rotulo honesto
--
-- O QUE MUDA, e por que:
--
-- 1. O motivo `sem_captura_cobranca` era um BALDE com quatro causas coladas, e
--    TRES delas nao sao lacuna nossa. O card dizia "falta dado nosso" para uma
--    venda que o Mercado Livre ainda nem faturou. A defasagem entre a venda e a
--    emissao da cobranca foi MEDIDA no CFFE: mediana 1 dia, maximo 18 — por
--    isso a constante `dias_defasagem_cffe`, nomeada no `cfg`.
--
-- 2. `tipo_caso` deixa de ser `tipo_calc` cru. D-239-01, ditado pelo Wesley:
--    "ele e o ditador da verdade, e para ditar ele precisa provar". Rotulo
--    afirmativo exige linha ACIONAVEL ou as TRES linhas do card fechadas —
--    esperado, recebido e diferenca. O resto sai `repasse_em_aberto`.
--    Mesma mecanica que o 239-01 aplicou ao frete.
--
-- 🔴 CORPO VIVO, lido do banco com pg_get_functiondef e transformado por
-- ancoras textuais. Nenhum byte veio do repositorio: foi assim que
-- `get_cashflow` regrediu R$ 30.372,11 nesta casa.
--
-- 🔴 O QUE ESTA MIGRATION NAO FAZ: nao liga regua de acusacao nenhuma, nao
-- toca `conciliacao_config`, nao escreve uma linha em tabela nenhuma. A fase
-- 239 PROVA; quem decide acusar e o Wesley, com o numero na frente.
-- ════════════════════════════════════════════════════════════════════════════
...
select k.id                                                      as caso_id,
       ...
       case
         ...
         when l.motivo in ('fora_da_janela_de_ingestao','sem_captura_cobranca',
                           ...
                           'captura_nunca_tentada','captura_com_erro') then 'nosso'
         else 'nenhuma'
       end                                                       as fila,
...
-- ── ACL: `create or replace` NAO preserva o grant desta base ────────────────
...
-- ── Guardas que ABORTAM a migration ────────────────────────────────────────
-- Comentario nao impede nada. Estas duas checam o estado REAL depois da
-- recriacao: uma funcao de tenant que nasca DEFINER e IDOR com `p_org_id` na
-- mao do chamador, e um grant escrito nao e um grant vigente.
do $guarda$
declare
  v_definer boolean;
begin
  ...
  if v_definer is null then
    raise exception '239-04: conciliacao_base_linhas nao existe depois da recriacao';
  end if;
  if v_definer then
    raise exception '239-04: conciliacao_base_linhas nasceu com privilegio do DONO (DEFINER) — e IDOR com p_org_id';
  end if;

  if not has_function_privilege('authenticated',
        'public.conciliacao_base_linhas(uuid, integer)', 'execute') then
    raise exception '239-04: authenticated perdeu o execute — a tela ficaria vazia sem erro';
  end if;
  if has_function_privilege('anon',
        'public.conciliacao_base_linhas(uuid, integer)', 'execute') then
    raise exception '239-04: anon executa a conciliacao — leitor nao autenticado veria caso de tenant';
  end if;
end
$guarda$;

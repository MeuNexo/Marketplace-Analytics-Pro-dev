...
serve(async (req) => {
  ...
  try {
    ...
    // ── refresh_all ───────────────────────────────────────────────────────────
    // Chamado pelo pg_cron a cada 90min para renovar todos os tokens Tiny próximos de expirar
    if (action === "refresh_all") {
      const admin = createClient(SUPABASE_URL, SERVICE_KEY);
      const now = Math.floor(Date.now() / 1000);
      const threshold = now + 30 * 60; // renova se expira em menos de 30min

      const { data: rows } = await admin
        .from("ml_tokens")
        .select("ml_user_id, tiny_refresh_token, tiny_expires_at")
        .not("tiny_refresh_token", "is", null)
        .lt("tiny_expires_at", threshold);

      if (!rows || rows.length === 0) return json({ success: true, refreshed: 0 });
        ...
        try {
          const tokenResp = await fetch(TINY_TOKEN_URL, {
            ...
            body: new URLSearchParams({
              ...
              client_id: TINY_APP_ID,
              client_secret: TINY_APP_SECRET,
              refresh_token: row.tiny_refresh_token,
            }),
          });
          ...
          await admin.from("ml_tokens").update({
            tiny_access_token: tokenData.access_token,
            tiny_refresh_token: tokenData.refresh_token ?? row.tiny_refresh_token,
            tiny_expires_at: now + (tokenData.expires_in ?? 3600),
          }).eq("ml_user_id", row.ml_user_id);
          refreshed++;
        } catch { /* continua */ }
      }

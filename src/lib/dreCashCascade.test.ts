...
describe("buildDreCashCascade — Test 1: guardrail entrada/previsao/excluido", () => {
  it("linhas de entrada/previsao e saida com bloco excluido NUNCA entram na soma de saídas", () => {
    const saidaRows: DreCashRow[] = [
      saidaRow("pessoal", "Salários", 5000),
      saidaRow("estrutura", "Aluguéis e condomínio", 2000),
    ];
    const cSemContaminacao = buildDreCashCascade(saidaRows);

    const rowsComContaminacao: DreCashRow[] = [
      ...saidaRows,
    ...
    const cComContaminacao = buildDreCashCascade(rowsComContaminacao);

    const totalSaidasSem = cSemContaminacao.saidas.reduce((s, b) => s + b.total, 0);
    const totalSaidasCom = cComContaminacao.saidas.reduce((s, b) => s + b.total, 0);
    expect(totalSaidasCom).toBe(totalSaidasSem);
    expect(totalSaidasCom).toBe(7000);

    // "excluido" nunca aparece como bloco de saída — estruturalmente impossível
    // (SAIDA_BLOCOS não o contém), mas provamos que o total dele (50000) não
    // vaza para nenhum subtotal.
    expect(cComContaminacao.saidas.map((b) => b.bloco)).not.toContain("excluido");
    expect(cComContaminacao.resultadoOperacional).not.toBe(90000 - 7000 - 50000);
  });
});

describe("buildDreCashCascade — Test 2: matemática da cascata", () => {
  it("resultadoOperacional = liquido − Σ(6 blocos); resultadoCaixa = resultadoOperacional − financeiro", () => {
    ...
    const totalSeisBlocos = 4000 + 27852 + 3000 + 2103 + 15715 + 7360; // 60030
    expect(c.resultadoOperacional).toBe(113000 - totalSeisBlocos); // 52970
    expect(c.financeiro).toBe(20027);
    expect(c.resultadoCaixa).toBe(52970 - 20027); // 32943

    // As 6 linhas de saída SEMPRE aparecem, mesmo blocos sem dados (total 0).
    expect(c.saidas.map((b) => b.bloco)).toEqual([...SAIDA_BLOCOS]);
    expect(c.saidas.every((b) => typeof b.total === "number")).toBe(true);
  });

  it("blocos sem linhas aparecem com total 0 — a cascata sempre mostra as 6 linhas", () => {
    const rows: DreCashRow[] = [
      entradaRow("liquido", 1000),
      saidaRow("pessoal", "Salários", 500),
    ];
    const c = buildDreCashCascade(rows);
    expect(c.saidas).toHaveLength(6);

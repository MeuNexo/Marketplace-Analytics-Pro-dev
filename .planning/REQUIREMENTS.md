...
- [x] **REPL-01**: Cálculo de reposição numa RPC `get_replenishment` server-side (`SECURITY INVOKER`, escopada por org, sem `org_id` em parâmetro — anti-IDOR), paginável; nada de compute pesado no front
- [x] **REPL-02**: Estoque atual = `ml_inventory_cache` (tudo no ML: Full + anúncios), read-only da fonte — não digitado pelo usuário
- [x] **REPL-03**: Venda/dia = média de vendas REAIS numa janela (default 30d) — não a curva de preço simulada
- [x] **REPL-04**: Modelo de ponto de reposição: `ponto = venda_dia×lead_time + estoque_seg`; `alvo = venda_dia×meta_cobertura + estoque_seg`; **só sugere compra quando estoque_atual ≤ ponto** (resolve "sugerir o que já tem")
- [x] **REPL-05**: Tabela `replenishment_params` (lead_time, meta_cobertura, safety, MOQ, pack) configurável **global + override por marca/fornecedor**, precedência marca > fornecedor > global
- [x] **REPL-06**: Sugestão respeita MOQ (mínimo do fornecedor) e múltiplo de embalagem (arredonda pra cima)
- [x] **REPL-07**: Custo nulo — sugere a quantidade mesmo sem custo, mas marca `custo_ausente` e não calcula valor R$ (causa conhecida: marcas de revenda sem custo no Tiny)
- [x] **REPL-08**: Sem giro — `venda_dia = 0` na janela → não sugere compra; flag `sem_giro` se houver estoque
- [x] **REPL-09**: v1 NÃO desconta compras "a chegar" (OC/trânsito); a tela exibe aviso explícito dessa limitação
- [x] **REPL-10**: A tela substitui os inputs digitados por colunas read-only da fonte (estoque, venda/dia, cobertura atual, ponto de reposição, sugestão, valor estimado) + flags + parâmetros usados (com origem global/marca)

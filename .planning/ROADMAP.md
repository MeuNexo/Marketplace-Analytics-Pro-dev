...
- [ ] 83-01-PLAN.md — [W1] Fundação backend+lógica pura: migration DROP+CREATE de `get_margin_with_ads_by_product` com coluna `marca` + `mcoHealth.ts` (constante `MCO_SAUDAVEL_PCT` + `classifyMcoHealth` semáforo) + `soldProductsMcoAgg.ts` (agregação pós-ads por marca/anúncio) — tudo com vitest

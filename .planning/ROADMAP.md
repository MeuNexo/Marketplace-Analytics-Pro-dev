...
- [ ] **Phase 67: Compras v3 — Reposição mais esperta (tendência + lead time real)** — A reposição da `/compras` deixa de usar só a **média simples** da janela de vendas e o **lead time fixo** dos params: passa a ponderar a velocidade por **tendência/sazonalidade** e/ou derivar o **lead time real por fornecedor** do histórico de OCs (`data_pedido`→`data_entrega`), tornando ponto de reposição e alvo mais precisos. Última peça do roadmap v2 da `/compras`. **A planejar — método e fontes a definir na discussão (SMART-01..04 provisórios).**

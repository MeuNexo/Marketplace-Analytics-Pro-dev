...
- **⚠️ Régua de imposto (CORRIGIDA 07-06 após ver os dados — ver `90-DATA-FINDINGS.md`):** a `dataCompetencia` do Tiny marca o **mês do PAGAMENTO**, não o da venda. Guia de competência **C** cobre as **vendas de C−1**. Portanto a DRE do mês de venda **`S` usa a guia de competência `S+1`** (deslocamento). A ideia anterior de "usar direto sem deslocar" está **REVOGADA** por decisão do Wesley.
- **Gatilho provisão→real (CORRIGIDO):** mês de venda `S` é **FECHADO** (usa imposto real) só quando a guia de competência `S+1` é **real apurada** = `status='paid'` **E sem placeholder** (categorias com valor > ~R$1; o Tiny tem previsões `pending` de Jul-Dez copiadas de maio + junho com PIS/COFINS = R$0,01 que NÃO podem contar). Senão → **ABERTO** → estimativa (provisão). A RPC `get_dre_operational_by_competence` **hoje não expõe nem filtra `status`** → precisa estender.

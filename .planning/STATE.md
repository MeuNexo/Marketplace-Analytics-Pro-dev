...
- **Wave 3 (57-04) — DEPLOY PENDENTE (bloqueado em SUPABASE_ACCESS_TOKEN):** EF `nexo-chat` pronta no repo mas NÃO deployada — CLI sem token, e deploy via MCP exigiria transcrever playbooks.ts (50KB) à mão. Pré-req do vault OK (get_app_secret('GEMINI_API_KEY') não-nulo). Comando: `SUPABASE_ACCESS_TOKEN=… supabase functions deploy nexo-chat --project-ref ckcdevcxgvueywivefgx`. Depois: preview Vercel + validação E2E Wesley (NEXO-01..07).

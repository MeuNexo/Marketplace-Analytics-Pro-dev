...
- **PENDENTE p/ próxima sessão (EF + deploy):** escrever EF `consultor-llm` com a config acima (substitui a chamada Anthropic do 53-01 plan), `GEMINI_API_KEY` via vault (`vault.create_secret` + RPC `get_app_secret` SECURITY DEFINER service_role-only, OU secret no dashboard); deploy via MCP; smoke-test com org real de insights. UI 53-02 depois. ⚠️ Wesley deve ROTACIONAR a key (exposta no transcript).
